...
static inline u8g2_esp32_i2c_ctx_t *get_ctx(void) { return s_default_ctx; }
...
static esp_err_t i2c_recreate_device_if_needed(u8g2_esp32_i2c_ctx_t *ctx, uint8_t addr_7bit)
{
    ...
    esp_err_t err = i2c_master_bus_add_device((i2c_master_bus_handle_t)ctx->bus_handle, &dev_cfg,
...
uint8_t u8x8_byte_esp32_hw_i2c(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr)
{
    ...
    switch (msg)
    {
        ...
        case U8X8_MSG_BYTE_START_TRANSFER:
        {
            ...
            if (err != ESP_OK)
            {
                ESP_LOGE(TAG, "I2C set address 0x%02X failed: %s", i2c_addr_7bit,
                         esp_err_to_name(err));
                return 0;
            }
        ...
        case U8X8_MSG_BYTE_SEND:
        {
            ...
            while (remaining > 0)
            {
                ...
                {
                    esp_err_t err =
                        i2c_master_transmit((i2c_master_dev_handle_t)ctx->dev_handle, ctx->tx_buf,
                                            ctx->tx_len, ctx->cfg.timeout_ms);
                    if (err != ESP_OK)
                    {
                        ESP_LOGE(TAG, "i2c_master_transmit (partial) failed: %s",
                                 esp_err_to_name(err));
                        ctx->tx_len = 0;
                        return 0;
                    }
        ...
        case U8X8_MSG_BYTE_END_TRANSFER:
        {
            ...
            esp_err_t err = i2c_master_transmit((i2c_master_dev_handle_t)ctx->dev_handle,
                                                ctx->tx_buf, ctx->tx_len, ctx->cfg.timeout_ms);
            ctx->tx_len = 0;
            if (err != ESP_OK)
            {
